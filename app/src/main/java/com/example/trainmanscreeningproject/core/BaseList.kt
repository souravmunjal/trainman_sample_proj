 ...
 */
fun <T> RecyclerView.bind(items: List<T>, @LayoutRes singleLayout: Int = 0, singleBind: (View.(item: T, position: Int) -> Unit)): FastListAdapter<T> {
    layoutManager = LinearLayoutManager(context)
    return FastListAdapter(items.toMutableList(), this).map(singleLayout, { true }, singleBind)
}
...
fun RecyclerView.update() {
    (adapter as? FastListAdapter<*>)?.notifyDataSetChanged()
}

fun RecyclerView.removeItemAtPosition(position: Int) {
    (adapter as? FastListAdapter<*>)?.remove(position)
}

fun RecyclerView.removeAll() {
    (adapter as? FastListAdapter<*>)?.removeAll()
    (adapter as? FastListAdapter<*>)?.notifyDataSetChanged()
}

open class FastListAdapter<T>(private var items: MutableList<T>, private var list: RecyclerView) : RecyclerView.Adapter<FastListViewHolder<T>>() {

    private inner class BindMap(val layout: Int, var type: Int = 0, val bind: View.(item: T, position: Int) -> Unit, val predicate: (item: T) -> Boolean)
     ...
     * so you can just use the XML names of the views inside your layout to address them.
     */
    fun map(@LayoutRes layout: Int, predicate: (item: T) -> Boolean, bind: View.(item: T, position: Int) -> Unit): FastListAdapter<T> {
...
class FastListViewHolder<T>(override val containerView: View, val holderType: Int) : RecyclerView.ViewHolder(containerView), LayoutContainer {
    fun bind(entry: T, func: View.(item: T, position: Int) -> Unit) {
        containerView.apply {
